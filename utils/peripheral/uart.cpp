...
void getCalibrationParameters( void )
{
    ...
    // Radiometric gain
    readRegister( 0x2028, 2, (uint8_t *)&r_data );
    printf("\n1. b2b1: 0x%04x\r\n", r_data);
    g_stThCoef.RadioMetricGain = (float)r_data / 16384;
    printf("RadioMetricGain: %f\r\n", g_stThCoef.RadioMetricGain);

    // PWM target
    readRegister( 0x207D, 1, (uint8_t *)&r_data );
    g_stThCoef.pwmtarget = r_data * 256;
    printf("\n2. b3: 0x%02x\nPWM target: %f\r\n", r_data, g_stThCoef.pwmtarget);
    ...
    if ( !strcmp( (char *)header, "data" ) )  {
        ...
        for ( int idx = 0; idx < 5; idx++ )  {
            memcpy( (uint8_t *)&g_stThCoef.t[idx], &cal_arr[4 + 4*idx], 4 );
            printf("%f  ", g_stThCoef.t[idx]);
        }
        printf("]\r\n");
    }
}
...
void calcGainOffset( void )
{
    ...
    printf("\nboard_t: %d, shutter_t: %d\r\n", board_t, shutter_t);

    shuttertemp = (float)shutter_t / 100.0;
    g_stThCoef.Tcam = shuttertemp + 273.15;

    double coef[3];
    coef[0] = 1 - g_stThCoef.t[0] * (g_stThCoef.Tcam - 320);
    coef[1] = 1 - g_stThCoef.t[2] * (g_stThCoef.pwmtarget - g_stThCoef.t[3] / 2) / g_stThCoef.t[3];
    g_stThCoef.GT = g_stThCoef.RadioMetricGain * coef[0];
    coef[2] = g_stThCoef.GT / g_stThCoef.Tcam;
    printf("\tcoef: %f %f %f\r\n", coef[0], coef[1], coef[2]);
    g_stThCoef.rgain = -coef[2] / g_stThCoef.Tcam * coef[1];
    g_stThCoef.roffset = pow( g_stThCoef.Tcam - g_stThCoef.t[1], 4 );

    printf("{Tcam, GT}: {%f, %f}\r\n", g_stThCoef.Tcam, g_stThCoef.GT);
    printf("{gain, offset}: {%fe-6, %f}\r\n", g_stThCoef.rgain*1000000, g_stThCoef.roffset);
}
...
float calcTemp( unsigned short frVal )
{
    ...
    readRegister( 0x2D000000, 4, (uint8_t *)&r_data );
    v2 = ( 0x01 == r_data ) ? frVal - 32767 : 32678 - frVal;

    coef = (double)v2 / g_stThCoef.rgain;
    printf("\n4. v2: %d\r\nv2/rgain: %f\r\n", v2, coef);

    return ( pow( coef + g_stThCoef.roffset, 0.25 ) - 273.15 );
}
...
void ThermalProcess( void )
{
    ...
    char fw_ver_str[64];
    readRegister( 0x04, 64, (uint8_t *)fw_ver_str );
    printf("%s\r\n", fw_ver_str);
    readRegister( 0x020C, 64, (uint8_t *)fw_ver_str );
    printf("F/W v%s\r\n", fw_ver_str);
    ...
    printf("\nagc_clahe:%08x, colormap:%08x, gain_offset:%08x\r\n",
            val_agc_clahe, val_colormap, val_gain_offset
            );

#if 1	//JDBG
#define COORD_ROW	320
#define COORD_COL	240
    uint32_t w_data = 1, roi_data;
    writeRegister( 0x2D000000, 4, (uint8_t *)&w_data );
    w_data = (COORD_ROW << 16) | COORD_COL;
    writeRegister( 0x2D000008, 4, (uint8_t *)&w_data );
    readRegister( 0x2D00002C, 4, (uint8_t *)&roi_data );
    printf("ROI_Data(%d, %d): 0x%08x\r\n", COORD_ROW, COORD_COL, roi_data);
#endif
    ...
    {
    unsigned short frame_raw = (unsigned short)roi_data;
    printf( "raw 0x%04x, temp. %f\r\n", frame_raw, calcTemp(frame_raw) );
    }
}
#endif
