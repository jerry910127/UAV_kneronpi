...
int draw_display_image(cv::Mat *cv_img_display, const char *strImgFPS, const char *strInfFPS)
{
    ...
    if (KDP2_INF_ID_APP_YOLO == header_stamp->job_id) {
        ...
        // 其他資訊
#if 1
        cv::putText(*cv_img_display, strImgFPS, {10, cv_img_display->rows - 10},  cv::FONT_HERSHEY_COMPLEX_SMALL, 1.5, {50, 50, 255}, 1);
        cv::putText(*cv_img_display, strInfFPS, {330, cv_img_display->rows - 10},  cv::FONT_HERSHEY_COMPLEX_SMALL, 1.5, {50, 50, 255}, 1);
...
#endif
        {
            ...
            if (!cv_image_display.empty() && !cv_image_display_WEBCAM.empty()) {
                ...
                if ( !cv_image_display_WEBCAM.empty() )
                {
                    ...
                    fused_display = cv_image_display;
                    alpha_idx = 0;
                    alpha_ir  = kAlphaLevels[alpha_idx];
                }
#endif
