...
void *example_display_liveview_thread(void *)
{
    ...
    while (true == _blDisplayRunning) {
        ...
        if (false == cv_image_display.empty()) {
            ...
            cv::rotate(cv_image_display, cv_image_display, cv::ROTATE_90_CLOCKWISE);  
        ...
        if (now != last_save_time)
        {
            ...
            // char filename_sensor[128];
            char filename_webcam[128];
            // sprintf(filename_sensor, "/work/image/rgb/sensor_%s.jpg", time_str);
            // sprintf(filename_webcam, "/work/image/ir/webcam_%s.jpg", time_str);
            ...
            if (!cv_image_display.empty())
                cv::imwrite(filename_webcam, cv_image_display);
            // if (!cv_image_display_WEBCAM.empty())
            //     cv::imwrite(filename_webcam, cv_image_display_WEBCAM);
            // 儲存疊合畫面
            // if (!fused_display.empty()) {
            //     char filename_fused[128];
            //     sprintf(filename_fused, "/work/image/fused/fused_%s.jpg", time_str);
            //     cv::imwrite(filename_fused, fused_display);
            // }
