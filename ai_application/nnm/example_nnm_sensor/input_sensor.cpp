...
{
	VMF_VI_OPTION_T atViOpt[VMF_VI_MAX_SEN_NUM];
	unsigned dwSenCnt = 1;
	
	memset(&atViOpt[0], 0, sizeof atViOpt);

	for (unsigned i = 0; i < dwSenCnt; i++) {
		VMF_VI_OPTION_T *ptViOpt = &atViOpt[i];

		//! The first channel index of the sensor
		//! Linear mode => 1 sensor (1 channel)
		//! Fusion mode => 1 sensor (n channel)
		//! Dual mode   => 2 sensor (1 channel per sensor)
		//! No matter what the mode is,
		//! there always is a channel index equal to the corressponding senor sequence.
		ptViOpt->dwIndex = i;
		ptViOpt->eOptionFlag = VI_OPTION_FRAME_RATE;
		ptViOpt->pData = (void *) &dwFrameRate;
	}

	VMF_VSRC_SetVIOptions(g_ptVsrcHandle, &atViOpt[0], dwSenCnt);
}
#endif
...
void *example_sensor_image_thread(void *arg)
{
    ...
    gVenc.config.fnSrcConnectFunc = (VMF_SRC_CONNECT_FUNC) VMF_BIND_Request;
    gVenc.config.pBind = g_ptBind;
    snprintf( tmpName, 30, SRB_DEFAULT_PREFIX"_%d", 1 );

    if ( 0 != venc_init( &gVenc, tmpName ) )  {
        printf( "[JDBG] venc_init stream failed!\r\n" );
    }
#endif
...
#ifdef VENC_API_H
    int ret = venc_release(&gVenc);
    if (ret)  {
        printf("[JDBG] venc_release stream failed, ret = %d\r\n", ret);
    }
#endif
