...
void *example_webcam_input_thread(void *arg)
{
    ...
    //mmlab rwei宣告一個用來省運算的     int disp_frame_cnt = 0;
...
//不需要            y16 = y16_crop.clone();


            if( disp_frame_cnt>=8 ){
                disp_frame_cnt=0;

                const int cx = y16_crop.cols / 2;
                const int cy = y16_crop.rows / 2;
                const int qx = y16_crop.cols / 4;
                const int qy = y16_crop.rows / 4;

                const int x0 = std::max(0, cx - qx);
                const int y0 = std::max(0, cy - qy);
                const int x1 = std::min(y16_crop.cols, cx + qx);
                const int y1 = std::min(y16_crop.rows, cy + qy);
                cv::Rect roi(x0, y0, x1 - x0, y1 - y0);
                cv::Mat center_crop = y16_crop(roi);
                cv::Scalar mean, stddev;
                cv::meanStdDev(y16_crop(roi), mean, stddev);

                int x_min_d =(int)(mean[0] - (3 * stddev[0]));
                int x_max_d =(int)(mean[0] + (3 * stddev[0]));

                // 邊界防護：轉為無號整數，並確保落在 0~65535 之間
                uint32_t x_min = (x_min_d < 0.0) ? 0 : (uint32_t)x_min_d;
                uint32_t x_max = (x_max_d > 65535.0) ? 65535 : (uint32_t)x_max_d;

                // 防呆：若影像毫無對比度
            // 防呆：若影像毫無對比度
                if (x_max <= x_min) {
                    // 放棄動態拉伸，強制將邊界設為預設值 (0 與 255)
                    x_min = 0;
                    x_max = 255;
                    
                    // 【關鍵】：這裡絕對不能寫 return！
                    // 讓程式繼續往下走，進入原有的 AGC 轉換迴圈
                }

                // 4. 計算反轉公式：最暗(x_min)變白(255)，最亮(x_max)變黑(0)
                double alpha = -255.0 / (double)(x_max - x_min); // 斜率加上負號
                double beta  = -(double)x_max * alpha;           // 基準點改用 x_max 來推算截距

                cv::Mat y8_out;
                y16_crop.convertTo(y8_out, CV_8UC1, alpha, beta);
                disp_frame_cnt++;
            }
        ...
        cv::Mat y8_out;
        cv::cvtColor(y8_out, cv_img_to_be_sent, cv::COLOR_BGR2RGBA);  //改成y8_out 出去
