...
UAV_ROOT = os.path.dirname(SCRIPT_DIR) if os.path.basename(SCRIPT_DIR) in ["daemon", "bin", "20260726_dev"] else SCRIPT_DIR
DEV_BIN_DIR = os.path.join(UAV_ROOT, "bin")
BIN_DIR = DEV_BIN_DIR
TIMEOUT_SEC = 300.0
...
class UAVDaemon:
    ...
    def ensure_test_video(self):
        mp4_path = None
        candidate_dirs = [
            os.path.join(UAV_ROOT, "daemon", "test_video"),
            os.path.join(UAV_ROOT, "bin", "test_video"),
            os.path.join(SCRIPT_DIR, "test_video"),
            os.path.join(UAV_ROOT, "test_video"),
        ]
