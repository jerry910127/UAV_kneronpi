...
int loadConfig(const char* HostVerifyConfigPath, EXAMPLE_WEBCAM_INIT_OPT_T* pExampleWebCamInit)
{
	dictionary* ini = NULL;
	struct stat info;
        ...
        switch (ch) {
            case 'c':
                if(pszConfigPath == NULL) {
                    ...
                    printf("pszHostWebCamConfigPath = %s\n", pszConfigPath);
                }
                break;
...
int main (int argc, char* argv[])
{
    ...
    char* pszHostWebCamConfigPath = NULL;
    const char* kMiramarCtrlDev = "/dev/ttyACM1";  // Miramar control port (edit here)
    ...
    if (argc > 1) {
        pszHostWebCamConfigPath = parse_argument(argc, argv);
        if(pszHostWebCamConfigPath == NULL)   //parse argument
            goto EXIT;

        if(0 != loadConfig(pszHostWebCamConfigPath, &ExampleWebCamInit))                  // load ini file
            goto EXIT;

    }
    else {
        if(0 != loadConfig(EXAMPLE_WEBCAM_CONFIG_PATH, &ExampleWebCamInit))                // load ini file
        	goto EXIT;
    }
    ...
    // --- Miramar thermal camera control (from peripheral/uart.cpp) ---
    // Default control port:
    //   Change this path if your control interface is on a different node.
    //   (Also used by display_liveview.cpp for runtime toggling.)
    // const char* kMiramarCtrlDev = "/dev/ttyACM1";

    // Keep control port open during runtime so we can toggle AGC/CLAHE via keyboard.
    if (0 == miramar_open(kMiramarCtrlDev)) {
        // Optional: set initial defaults here (comment out if you want to keep current camera state)
        //   AGC enable: 1=ON, 0=OFF
        //   CLAHE enable: 1=ON, 0=OFF
        miramar_set_agc_clahe(1, 0);
        printf("[miramar] control ready on %s (press 'A'/'C' in display window)\n", kMiramarCtrlDev);
    } else {
...
//SIGSEGV
    ...
    VMF_NNM_Load_Model_From_File(ExampleWebCamInit.pszModelPath);

    VMF_NNM_Fifoq_Manager_Allocate_Buffer(IMAGE_BUFFER_COUNT, IMAGE_BUFFER_SIZE, RESULT_BUFFER_COUNT, RESULT_BUFFER_SIZE);

    pthread_create(&task_webcam_image_handle, NULL, example_webcam_input_thread, &ExampleWebCamInit);
    pthread_create(&task_send_inf_handle, NULL, example_send_inf_thread, &ExampleWebCamInit.dwJobId);
...
EXIT:
    ...
    if (pszHostWebCamConfigPath)
        free(pszHostWebCamConfigPath);
