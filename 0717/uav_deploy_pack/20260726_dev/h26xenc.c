...
int g_bSrtEnabled = 0;
char *g_szSrtIp = NULL;
int g_dwSrtPort = 9000;

static void send_srt_headers(void)
{
    if (!g_bSrtEnabled || g_srt_sock == SRT_INVALID_SOCK || !g_ptStreamHdr) return;
    
    if (g_eCodec == 1) { // H.265
        if (g_ptStreamHdr->dwVpsSize > 0) {
            srt_send(g_srt_sock, (const char*)g_ptStreamHdr->abyVpsData, g_ptStreamHdr->dwVpsSize);
        }
    }
    if (g_ptStreamHdr->dwSpsSize > 0) {
        srt_send(g_srt_sock, (const char*)g_ptStreamHdr->abySpsData, g_ptStreamHdr->dwSpsSize);
    }
    if (g_ptStreamHdr->dwPpsSize > 0) {
        srt_send(g_srt_sock, (const char*)g_ptStreamHdr->abyPpsData, g_ptStreamHdr->dwPpsSize);
    }
}

static void send_srt_data(const void* data, int size)
{
    if (!g_bSrtEnabled) return;
    ...
    if (g_srt_sock == SRT_INVALID_SOCK) {
        ...
        if (g_srt_sock != SRT_INVALID_SOCK) {
            ...
            printf("[SRT] Reconnected successfully to %s:%d!\n", g_szSrtIp, g_dwSrtPort);
            send_srt_headers();
        } else {
            return;
        }
    ...
    if (ret == SRT_ERROR) {
        ...
        printf("[SRT] Send failed: %s (error %d)\n", srt_getlasterror_str(), err);
        if (err == SRT_ECONNLOST || err == SRT_ENOCONN || err == SRT_ETIMEOUT) {
            srt_close(g_srt_sock);
            g_srt_sock = SRT_INVALID_SOCK;
        }
    }
...
static int do_h26xe(VMF_FRAME_BUF_T* pVirtFrameBuf, VMF_FRAME_BUF_T* pPhysFrameBuf, void* ptEncBuff, unsigned int* bIdr,  FILE* fp)
{
    ...
    if (g_bSrtEnabled && enc_bytes) {
        send_srt_data(ptEncBuff, enc_bytes);
    }
...
static void* h26xenc_loop(void* data __attribute__((unused)))
{
    ...
    codec_initopt.dwMaxWidth = g_dwTargetW;
    codec_initopt.dwMaxHeight = g_dwTargetH;
    codec_initopt.dwCropX = codec_initopt.dwCropY = 0;
    printf("[%s] Ratio:%d, ImgWH:%dx%d, EncWH:%dx%d, Stride:%d\r\n", __func__,
            codec_initopt.dwCompressionRatio,
            codec_initopt.dwImgWidth, codec_initopt.dwImgHeight,
            codec_initopt.dwEncWidth, codec_initopt.dwEncHeight,
            ((codec_initopt.dwEncWidth + 31) & (~31))
            );
    ...
    if (g_bSrtEnabled) {
        ...
        g_srt_sock = srt_create_socket();
        if (g_srt_sock == SRT_INVALID_SOCK) {
            printf("[SRT] Failed to create socket: %s\n", srt_getlasterror_str());
        } else {
            int yes = 1;
            int latency = 120;
            int message_api = 0;
            ...
            printf("[SRT] Connecting to %s:%d (caller mode)...\n", g_szSrtIp, g_dwSrtPort);
            if (srt_connect(g_srt_sock, (struct sockaddr*)&sa, sizeof sa) == SRT_ERROR) {
                printf("[SRT] Connection failed: %s. Will retry during streaming.\n", srt_getlasterror_str());
                srt_close(g_srt_sock);
                g_srt_sock = SRT_INVALID_SOCK;
            } else {
                printf("[SRT] Connected to %s:%d successfully!\n", g_szSrtIp, g_dwSrtPort);
                send_srt_headers();
            }
        }
    }
...
static int loadConfig(char *configPath)
{
	...
	do {
		tmp = iniparser_getstring(ini, "main:ssm_reader_pin", NULL);
        if (!tmp) {
            tmp = iniparser_getstring(ini, "venc:ssm_reader_pin", NULL);
        }
        if (tmp == NULL) {
        ...
        tmp = iniparser_getstring(ini, "main:srb_writer_pin", NULL);
        if (!tmp) {
            tmp = iniparser_getstring(ini, "venc:srb_writer_pin", NULL);
        }
        if (tmp == NULL) {
		...
		g_eCodec = iniparser_getint(ini, "main:codec_type", -1);
        if (g_eCodec == -1) {
            g_eCodec = iniparser_getint(ini, "venc:codec_type", 0);
        }

        g_bWriteEnable = iniparser_getint(ini, "main:enanble_write", -1);
        if (g_bWriteEnable == -1) {
            g_bWriteEnable = iniparser_getint(ini, "main:enable_write", -1);
        }
        if (g_bWriteEnable == -1) {
            g_bWriteEnable = iniparser_getint(ini, "venc:enable_write", 0);
        }

        g_dwTargetW = iniparser_getint(ini, "main:Resize_Width", -1);
        if (g_dwTargetW == -1) {
            g_dwTargetW = iniparser_getint(ini, "venc:Resize_Width", 0);
        }

        g_dwTargetH = iniparser_getint(ini, "main:Resize_Height", -1);
        if (g_dwTargetH == -1) {
            g_dwTargetH = iniparser_getint(ini, "venc:Resize_Height", 0);
        }

        const char *cam_tmp = iniparser_getstring(ini, "main:CameraPath", NULL);
        if (!cam_tmp) {
            cam_tmp = iniparser_getstring(ini, "nnm:CameraPath", "/dev/video0");
        }
        g_szCameraPath = strdup(cam_tmp);

        // Load SRT streaming configuration
        g_bSrtEnabled = iniparser_getint(ini, "main:srt_enabled", -1);
        if (g_bSrtEnabled == -1) {
            g_bSrtEnabled = iniparser_getint(ini, "venc:srt_enabled", 1); // default to 1
        }
        
        const char *srt_ip_tmp = iniparser_getstring(ini, "main:srt_ip", NULL);
        if (!srt_ip_tmp) {
            srt_ip_tmp = iniparser_getstring(ini, "venc:srt_ip", "192.168.168.24");
        }
        g_szSrtIp = strdup(srt_ip_tmp);
        
        g_dwSrtPort = iniparser_getint(ini, "main:srt_port", -1);
        if (g_dwSrtPort == -1) {
            g_dwSrtPort = iniparser_getint(ini, "venc:srt_port", 9000);
        }
